import { dosen, programStudi } from '@kampusia/db/schema';
...
type Write = Pick<typeof dosen.$inferInsert, 'userId' | 'programStudiId' | 'kodeDosen' | 'nidn' | 'nama' | 'isActive'>;
const selection = { ...getTableColumns(dosen), programStudi: { id: programStudi.id, kode: programStudi.kode, nama: programStudi.nama, isActive: programStudi.isActive } };
  ...
  return {
    async list(query: DosenQuery) {
      ...
      const where = and(
        ...
        query.search?.trim() ? or(ilike(dosen.kodeDosen, searchPattern(query.search)), ilike(dosen.nidn, searchPattern(query.search)), ilike(dosen.nama, searchPattern(query.search))) : undefined,
      );
      return db.transaction(async tx => {
        const data = await tx.select(selection).from(dosen).leftJoin(programStudi, eq(dosen.programStudiId, programStudi.id))
    ...
    async findById(id: string) {
      const [row] = await db.select(selection).from(dosen).leftJoin(programStudi, eq(dosen.programStudiId, programStudi.id)).where(eq(dosen.id, id));
      return row;
    },
