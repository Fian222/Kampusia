...
export async function fixture(tx: Transaction) {
  ...
  const accounts = await tx.insert(users).values(['MAHASISWA', 'MAHASISWA', 'AKADEMIK'].map((role, i) => ({ email: `${prefix.toLowerCase()}${i}@test.local`, passwordHash: 'unused-test-account', role: role as 'MAHASISWA' | 'AKADEMIK' }))).returning();
  const students = await tx.insert(mahasiswa).values(accounts.slice(0, 2).map((row, i) => ({ userId: row.id, nim: prefix + i, nama: prefix + i, programStudiId: program!.id, kurikulumId: curriculum!.id, angkatan: 2026 }))).returning();
  return { prefix, term, faculty: faculty!, program: program!, courses, curriculum: curriculum!, classes, room: room!, lecturer: lecturer!, slots, accounts, students, admin: accounts[2]!, user: accounts[0]!, other: accounts[1]! };
}
