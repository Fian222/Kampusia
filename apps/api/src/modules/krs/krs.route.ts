...
import { detailParams, emptyBody, krsQuery, reasonBody, selectionBody } from './krs.model';
...
export function krsRoutes(auth: AuthService, origin: string, service?: KrsService) {
  ...
  const admin = new Elysia({ prefix: '/krs' }).use(authorization(auth))
    ...
    .post('/:id/reject', async ({ user, params, body }) => ({ success: true as const, data: await get().reject(user, params.id, body.alasan) }), { params: idParams, body: reasonBody })
    .post('/:id/cancel', async ({ user, params, body }) => ({ success: true as const, data: await get().cancel(user, params.id, body.alasan) }), { params: idParams, body: reasonBody })
    .post('/:id/reopen', async ({ user, params }) => ({ success: true as const, data: await get().reopenApproved(user, params.id) }), { params: idParams, body: emptyBody });
  const student = new Elysia({ prefix: '/mahasiswa/me/krs' }).use(authorization(auth))
    ...
    .post('/:id/clear', async ({ user, params }) => ({ success: true as const, data: await get().clear(user, params.id) }), { params: idParams, body: emptyBody })
    ...
    .post('/:id/reopen', async ({ user, params }) => ({ success: true as const, data: await get().reopenRejected(user, params.id) }), { params: idParams, body: emptyBody });
  const adviser = new Elysia({ prefix: '/dosen/me/krs' }).use(authorization(auth))
    .onBeforeHandle(({ user, request }) => {
      requireRole(user, ['DOSEN']);
      if (!['GET', 'HEAD'].includes(request.method) && request.headers.get('origin') !== origin) throw new AuthError(403, 'Asal permintaan tidak diizinkan.');
    })
    .get('/', async ({ user, query }) => ({ success: true as const, ...await get().adviserList(user, query) }), { query: krsQuery })
    .get('/:id', async ({ user, params }) => ({ success: true as const, data: await get().adviserGet(user, params.id) }), { params: idParams })
    .post('/:id/approve', async ({ user, params }) => ({ success: true as const, data: await get().approve(user, params.id) }), { params: idParams, body: emptyBody })
    .post('/:id/reject', async ({ user, params, body }) => ({ success: true as const, data: await get().reject(user, params.id, body.alasan) }), { params: idParams, body: reasonBody })
    .post('/:id/reopen', async ({ user, params }) => ({ success: true as const, data: await get().reopenApproved(user, params.id) }), { params: idParams, body: emptyBody });
  return new Elysia().use(admin).use(student).use(adviser);
}
