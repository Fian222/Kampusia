...
export function semesterRoutes(auth: AuthService, origin: string, service?: SemesterService) {
  const getService = () => { if (!service) throw new MasterDataError(503, 'Layanan semester tidak tersedia.'); return service; };
  return new Elysia({ prefix: '/semester' }).use(authorization(auth))
    .onBeforeHandle(({ user, request }) => requireMasterData(user, request, origin))
    .get('/', async ({ query }) => ({ success: true as const, ...await getService().list(query) }), { query: semesterQuery })
