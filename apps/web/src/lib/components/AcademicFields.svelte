<script lang="ts">
  export type Field = { name: string; label: string; value?: string | number; type?: 'text' | 'number' | 'date'; min?: number; max?: number; maxlength?: number; options?: { value: string; label: string; disabled?: boolean }[] };
...
{#each fields as field}
  <label class="text-sm font-medium">{field.label}
    ...
    {:else}
      <input class="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2" name={field.name} type={field.type ?? 'text'} min={field.min} max={field.max} maxlength={field.maxlength} required value={values[field.name] ?? field.value ?? ''} />
    {/if}
  </label>
{/each}
