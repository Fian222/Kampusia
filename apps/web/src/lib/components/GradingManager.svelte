<script lang="ts">
  ...
  const scoreFeedback = (studentId: string, componentId: string) => form?.values?.mode === 'score-save' && form.values.mahasiswa_id === studentId && form.values.component_id === componentId ? form : undefined;
...
<section class="surface-panel mt-6 overflow-hidden">
  ...
  <div class="flex flex-wrap items-start justify-between gap-3">
    <div><p class="eyebrow">Evaluasi pembelajaran</p><h2 class="mt-2 text-xl font-bold">{area === 'akademik' ? 'Pengawasan Penilaian' : 'Penilaian'}</h2><p class="mt-1 text-sm leading-6 text-slate-500">{area === 'akademik' ? 'Tinjau konfigurasi, kelengkapan, dan hasil nilai kelas.' : 'Nilai kosong berarti belum dinilai; '} {#if area !== 'akademik'}<strong class="font-semibold text-slate-700">0.00 tetap merupakan nilai sah</strong>.{/if}</p></div>
    <div class="flex flex-wrap gap-2"><div class="rounded-xl bg-slate-100 px-4 py-2.5 text-sm"><span class="block text-xs text-slate-500">Bobot aktif</span><strong class="text-slate-900">{grading.summary.activeWeight}%</strong></div><div class="rounded-xl bg-brand-50 px-4 py-2.5 text-sm"><span class="block text-xs text-brand-700">Nilai lengkap</span><strong class="text-brand-900">{grading.summary.completeStudents}/{grading.summary.totalStudents}</strong></div><div class="rounded-xl bg-slate-100 px-4 py-2.5 text-sm"><span class="block text-xs text-slate-500">Status</span><strong class="text-slate-900">{grading.summary.finalized ? 'Final' : 'Belum final'}</strong></div></div>
  </div>
  ...
  {#if area === 'akademik' && !grading.summary.finalized}<div class="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-3.5 text-sm leading-6 text-slate-700"><strong>Mode pengawasan.</strong> Komponen, nilai, dan finalisasi biasa dikerjakan oleh Dosen pengajar pada ruang kelasnya.</div>{/if}

  <div class="mt-7 flex items-center justify-between gap-3"><h3 class="font-bold text-slate-900">Komponen nilai</h3>{#if grading.permissions.canManageComponents}<button type="button" class={button} onclick={() => { editingComponentId = undefined; componentOpen = true; }}><span class="inline-flex items-center gap-1.5"><Icon name="plus" size={14} /> Tambah komponen</span></button>{/if}</div>
  ...
  <div class="mt-3 max-h-[70vh] overflow-auto rounded-xl border border-slate-200"><table class="min-w-full text-left text-sm"><thead class="sticky top-0 z-20"><tr><th class="sticky left-0 z-30 min-w-56 border-r border-slate-200 bg-slate-50 p-3">NIM / Nama</th>{#each grading.components.filter((item: { isActive: boolean }) => item.isActive) as component}<th class="min-w-48 p-3">{component.nama}<br /><span class="font-normal normal-case tracking-normal">Bobot {component.bobot}%</span></th>{/each}<th class="min-w-40 p-3">Nilai akhir</th></tr></thead>
    <tbody class="divide-y divide-slate-100">{#each grading.data as row}<tr class="group align-top"><td class="sticky left-0 z-10 border-r border-slate-100 bg-white p-3 group-hover:bg-slate-50"><span class="font-semibold text-slate-900">{row.mahasiswa.nim}</span><br /><span class="text-slate-600">{row.mahasiswa.nama}</span></td>{#each grading.components.filter((item: { isActive: boolean }) => item.isActive) as component}{@const current = scoreFor(row, component.id)}<td class="p-3">
      {#if grading.permissions.canRecordScores}<form method="POST" action="?/grading" class="flex gap-2" use:enhance={submit}><input type="hidden" name="mode" value="score-save" /><input type="hidden" name="mahasiswa_id" value={row.mahasiswa.id} /><input type="hidden" name="component_id" value={component.id} /><input class={`${input} min-w-28 font-mono`} class:border-amber-400={current === null} class:bg-amber-50={current === null} name="nilai" value={scoreFeedback(row.mahasiswa.id, component.id)?.values?.nilai ?? current ?? ''} inputmode="decimal" placeholder="Belum dinilai" aria-label={`Nilai ${component.nama} ${row.mahasiswa.nama}`} /><button class={button} disabled={saving}>Simpan</button></form>{#if scoreFeedback(row.mahasiswa.id, component.id)?.message}<p class="mt-1 text-xs text-red-700" role="alert">{scoreFeedback(row.mahasiswa.id, component.id).message}</p>{/if}{:else}<span class:font-semibold={current === '0.00'} class:text-slate-400={current === null}>{current ?? 'Belum dinilai'}</span>
        {#if grading.permissions.canCorrect}<button type="button" class="mt-2 block text-xs text-amber-800" aria-label={`Koreksi ${component.nama} ${row.mahasiswa.nama}`} onclick={() => { correctionStudentId = row.mahasiswa.id; correctionComponentId = component.id; correctionOpen = true; }}>Koreksi administratif</button>{/if}
      {/if}
  ...
  {#if area === 'dosen' && !grading.summary.finalized}<div class="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-4"><h3 class="font-bold text-amber-950">Finalisasi</h3><p class="mt-1 text-sm leading-6 text-amber-900">{grading.finalization.message}</p>{#if grading.permissions.canFinalize}<p class="mt-2 text-sm leading-6 text-amber-900">Finalisasi bersifat atomik dan membekukan konfigurasi serta penyuntingan biasa. Kelas harus DITUTUP, bobot harus 100.00%, dan seluruh nilai harus lengkap.</p><form method="POST" action="?/grading" class="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between" use:enhance={submit}><input type="hidden" name="mode" value="finalize" /><label class="flex items-start gap-2 text-sm"><input class="mt-0.5" type="checkbox" name="confirm" value="yes" required /> Saya memahami dan mengonfirmasi finalisasi</label><button class={button} disabled={saving || grading.kelas.status !== 'DITUTUP' || grading.summary.activeWeight !== '100.00' || grading.summary.missingScores > 0 || grading.summary.totalStudents === 0}>Finalisasi Nilai</button></form>{/if}</div>{/if}
  </div>
</section>

{#if grading.permissions.canManageComponents}<Modal bind:open={componentOpen} title={`${editingComponent ? 'Edit' : 'Tambah'} Komponen Nilai`} description="Total bobot komponen aktif harus mencapai 100% sebelum finalisasi." closeDisabled={saving} width="md" onClose={() => { if (componentValues) void goto(page.url, { replaceState: true, noScroll: true, keepFocus: true }); }}>
...
{#if grading.permissions.canCorrect}<Modal bind:open={correctionOpen} title="Koreksi Nilai Resmi" description={correctionStudent && correctionComponent ? `${correctionStudent.mahasiswa.nama} · ${correctionComponent.nama}` : undefined} closeDisabled={saving} width="md" onClose={() => { if (correctionValues) void goto(page.url, { replaceState: true, noScroll: true, keepFocus: true }); }}>
