  ...
  <div class="flex flex-wrap items-start justify-between gap-4 p-5 sm:p-6">
    <div>
      ...
      <p class="mt-2 max-w-3xl text-sm text-slate-500">
        {area === 'akademik'
          ? 'Buka dan kelola pertemuan kelas. Pertemuan yang dibuka langsung tersedia bagi dosen pengajar untuk pencatatan absensi.'
    ...
    {#each meetings.data as row}
      <article class={`rounded-xl border p-4 ${row.status === 'SELESAI' ? 'border-slate-200 bg-slate-50/70' : row.status === 'DIBATALKAN' ? 'border-red-100 bg-red-50/30' : 'border-brand-100 bg-white'}`}>
        <div class="flex items-start justify-between gap-3">
          <div><p class="text-xs font-semibold uppercase tracking-[0.1em] text-slate-500">Pertemuan {row.nomorPertemuan}</p><h3 class="mt-1 font-bold text-slate-950">{formatAcademicDate(row.tanggal)}</h3><p class="mt-1 text-sm text-slate-500">{row.jamMulai.slice(0, 5)}–{row.jamSelesai.slice(0, 5)}{row.materi ? ` · ${row.materi}` : ''}</p></div>
        ...
        <div class="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-3">
          ...
          <div class="flex flex-wrap items-center gap-3">
            ...
            {#if area === 'akademik' && row.status === 'TERJADWAL'}
              <details><summary class="cursor-pointer text-sm font-semibold text-red-700">Batalkan</summary><form method="POST" {action} class="mt-2 min-w-56 rounded-lg border border-red-100 bg-white p-3" use:enhance={submit}><input type="hidden" name="mode" value="meeting-cancel" /><input type="hidden" name="pertemuan_id" value={row.id} /><label class="block text-xs"><input type="checkbox" name="confirm" value="yes" required /> Konfirmasi pembatalan</label><button class={`${button} mt-2`} disabled={saving}>Batalkan Pertemuan</button></form></details>
            {/if}
          </div>
        </div>
      </article>
