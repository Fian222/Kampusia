<script lang="ts">
  ...
  let { action, rooms, roomMeta, schedule, values, saving, submit }: { action: string; rooms: Detail['rooms']['data']; roomMeta: Detail['rooms']['meta']; schedule?: Detail['schedules']['data'][number]; values?: Record<string, string>; saving: boolean; submit: SubmitFunction } = $props();
