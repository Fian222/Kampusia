      ...
      <tbody>
        {#each data.records as row (row.id)}
          <tr class="border-b border-slate-100"><td class="p-3 font-medium">{row.kode}</td><td class="p-3">{row.nama}</td>
            ...
            <td class="p-3"><div class="flex gap-4"><a class="font-medium text-teal-800" href={href({ edit: row.id }) + '#master-form'}>Edit</a><button class="text-slate-600 disabled:opacity-50" disabled={saving} onclick={() => confirmation = row}>{row.isActive ? 'Nonaktifkan' : 'Aktifkan'}</button></div></td>
          </tr>
...
{#if isProgram && data.faculties}
  <section class="mt-6 rounded-xl border border-slate-200 bg-white p-5" aria-label="Cari fakultas">
