  ...
  {#if classes.length}
    <div class="overflow-x-auto">
      <table class="min-w-[780px] w-full text-left text-sm">
        ...
        <tbody class="divide-y divide-slate-100">{#each classes as kelas}<tr>
          ...
          <td class="px-4 py-4">{#each kelas.jadwal as slot}<p class="mb-1 last:mb-0"><span class="font-medium text-slate-800">{days[slot.hari]}, {slot.jamMulai.slice(0, 5)}–{slot.jamSelesai.slice(0, 5)}</span><span class="block text-xs text-slate-500">{slot.ruangan.kode} · {slot.ruangan.nama}</span></p>{:else}<span class="text-slate-400">Belum dijadwalkan</span>{/each}</td>
