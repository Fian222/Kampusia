...
<section class="surface-panel mt-6 overflow-hidden">
  <div class="flex flex-col gap-4 border-b border-slate-100 p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6"><div><h2 class="font-bold">Daftar kehadiran</h2><p class="mt-1 text-sm leading-6 text-slate-500"><strong class="font-semibold text-amber-800">Belum dicatat bukan ALPHA.</strong> Pilih ALPHA hanya bila memang menjadi hasil kehadiran.</p></div>
  {#if roster.pertemuan.status === 'TERJADWAL'}<form method="POST" class="flex flex-col gap-2 sm:items-end" use:enhance={submit}><input type="hidden" name="mode" value="complete" /><label class="flex items-center gap-2 text-xs"><input type="checkbox" name="confirm" value="yes" required /> Semua kehadiran sudah benar</label><button class={button} disabled={saving}>Selesaikan pertemuan</button></form>{/if}</div>
  <div class="overflow-x-auto"><table class="min-w-[900px] w-full text-left text-sm"><thead><tr><th class="px-4 py-3.5">NIM</th><th class="px-4 py-3.5">Nama</th><th class="px-4 py-3.5">Status</th><th class="px-4 py-3.5">Pencatatan kehadiran</th></tr></thead>
    <tbody class="divide-y divide-slate-100">{#each roster.data as row}<tr><td class="px-4 py-4 font-mono text-xs font-semibold text-slate-700">{row.mahasiswa.nim}</td><td class="px-4 py-4 font-semibold text-slate-900">{row.mahasiswa.nama}</td><td class="px-4 py-4">{#if !row.absensi}<Badge tone="warning">Belum dicatat</Badge>{:else}<Badge tone={tone(row.absensi.status)}>{row.absensi.status}</Badge>{/if}</td>
      <td class="px-4 py-4"><form method="POST" class="flex min-w-96 flex-wrap gap-2" use:enhance={submit}>
      ...
      </form></td></tr>{:else}<tr><td colspan="4" class="p-8 text-center text-slate-500">Tidak ada mahasiswa dengan KRS efektif.</td></tr>{/each}</tbody>
  </table></div>
</section>
