...
export async function loadProfiles(event: RequestEvent, kind: ProfileKind) {
  ...
  if (kind === 'mahasiswa') {
    const records = await read(client.mahasiswa.get({ query: filters }));
    ...
    return { ...common, kind, records: records.data, meta: records.meta, edit: edit?.data ?? null, curricula };
  }
  ...
  return { ...common, kind, records: records.data, meta: records.meta, edit: edit?.data ?? null, curricula: null };
}
  ...
  try {
    if (mode === 'status') {
      ...
      if (kind === 'mahasiswa') {
        ...
        const body = { ...common, nim: values.nim, program_studi_id: values.program_studi_id, kurikulum_id: values.kurikulum_id, angkatan, status: selectedStatus };
