...
export function isMasterDataPath(path: string) {
  return /^\/akademik\/krs(?:\/[^/]+)?\/?$/.test(path) || ['/akademik/fakultas', '/akademik/program-studi', '/akademik/mata-kuliah', '/akademik/kurikulum'].includes(path.replace(/\/$/, '')) || /^\/akademik\/kurikulum\/[^/]+\/?$/.test(path);
}
