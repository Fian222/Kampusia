...
<div class="mt-5 flex flex-wrap items-center gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-panel"><span class="inline-flex size-9 items-center justify-center rounded-lg bg-brand-50 text-brand-700"><Icon name="calendar" size={18} /></span><div><p class="text-xs font-medium text-slate-500">Semester aktif</p><p class="text-sm font-semibold text-slate-900">{data.history.activeSemester?.nama ?? 'Belum ditetapkan'}</p>{#if data.history.activeSemester}<p class="mt-1 text-xs text-slate-500">Periode KRS: {data.history.activeSemester.krsMulaiAt ? new Date(data.history.activeSemester.krsMulaiAt).toLocaleString('id-ID', { timeZone: 'Asia/Jakarta' }) : 'belum dijadwalkan'}{data.history.activeSemester.krsSelesaiAt ? ` – ${new Date(data.history.activeSemester.krsSelesaiAt).toLocaleString('id-ID', { timeZone: 'Asia/Jakarta' })}` : ''} WIB</p>{/if}</div>{#if data.history.activeSemester}<a class="ml-auto text-sm font-semibold text-brand-700 hover:text-brand-800" href={'?semester_id=' + data.history.activeSemester.id}>Buka semester aktif →</a>{/if}</div>
{#if data.selected && data.period && !data.period.open}<p class="my-4 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">Periode KRS {data.period.label.toLowerCase()}. Data tetap dapat dibaca, tetapi perubahan dan peninjauan tidak tersedia.</p>{/if}
...
{#if data.selected}
  {#if plan}
    <KrsSummary krs={plan} />
    {#if plan.status === 'DRAFT' && data.selected.semester.isActive && data.period?.open}
      ...
      {#if plan.details.some(item => item.status === 'AKTIF')}<KrsAction id={plan.id} mode="clear" label="Kosongkan KRS" explanation="Semua pilihan aktif akan dikosongkan. Anda dapat mengisi kembali KRS selama periode masih dibuka." />{/if}
    ...
    {:else if plan.status === 'DITOLAK'}
      <p class="my-4 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-800"><strong>KRS ditolak.</strong> {plan.alasanPenolakan ?? 'Tidak ada alasan yang tersimpan.'}</p>{#if data.period?.open}<KrsAction id={plan.id} mode="reopen" label="Perbaiki KRS" explanation="Kembalikan KRS ke DRAFT untuk koreksi dan pengajuan baru?" />{/if}
    {:else if plan.status === 'DIAJUKAN'}<p class="my-4 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">Menunggu persetujuan Dosen PA. Pilihan tidak dapat diubah.</p>
