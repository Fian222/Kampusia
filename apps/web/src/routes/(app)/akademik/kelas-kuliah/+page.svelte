...
<p class="mt-3 flex items-start gap-2 text-sm leading-6 text-slate-600"><Icon name="info" size={17} class="mt-0.5 shrink-0 text-amber-600" /> Kelas dibuka setelah dosen, kurikulum, dan jadwalnya siap. Lengkapi semuanya dari ruang kerja kelas.</p>
...
<section class={`${box} relative`} aria-busy={listPending}>
  ...
  <div class="mt-4 overflow-x-auto transition-opacity" class:opacity-80={listPending}><table class="w-full text-left text-sm"><thead class="border-b text-slate-500"><tr>{#each ['Mata kuliah', 'Kelas', 'Semester & program', 'Pengajar', 'Kapasitas', 'Status', 'Tindakan'] as label}<th class="p-3">{label}</th>{/each}</tr></thead>
    <tbody>{#each data.records.data as row}<tr class="border-b border-slate-100"><td class="p-3"><span class="font-mono text-xs text-slate-500">{row.mataKuliah.kode}</span><p class="font-semibold text-slate-900">{row.mataKuliah.nama}</p></td><td class="p-3"><p class="font-semibold">Kelas {row.namaKelas}</p><p class="mt-1 text-xs text-slate-500">{row.mataKuliah.sks} SKS</p></td><td class="p-3"><p>{row.semester.nama}</p><p class="mt-1 text-xs text-slate-500">{row.programStudi.nama}</p></td><td class="p-3"><p>{row.dosen?.map(item => item.dosen.nama).join(', ') || 'Belum ditugaskan'}</p></td><td class="p-3">{row.kapasitas} kursi</td><td class="p-3"><Badge tone={row.status === 'DIBUKA' ? 'success' : row.status === 'DIBATALKAN' ? 'danger' : 'neutral'}>{row.status}</Badge></td><td class="p-3"><div class="flex gap-2"><a class="action-primary" href={`/akademik/kelas-kuliah/${row.id}`}>Buka kelas</a><a class="action-secondary" aria-label={`Edit kelas ${row.mataKuliah.nama} ${row.namaKelas}`} href={editQueryHref(page.url, row.id)} data-sveltekit-noscroll data-sveltekit-keepfocus onclick={() => openEdit(row.id)}>Edit</a></div></td></tr>{:else}<tr><td colspan="7" class="p-8 text-center text-slate-500">Tidak ada kelas yang cocok.</td></tr>{/each}</tbody>
...
<Modal bind:open={formOpen} title={`${editModal.editing ? 'Edit' : 'Tambah'} Kelas Kuliah`} description={editModal.loading ? 'Menyiapkan data untuk disunting.' : undefined} closeDisabled={saving} width="lg" onClose={() => { const shouldClear = requestedEditId !== null || queryEditId || page.url.searchParams.has('modal') || form?.values?.mode === 'save'; requestedEditId = null; if (shouldClear) void goto(clearEditQueryHref(page.url), { replaceState: true, ...modalNavigationOptions }); }}>
  {#if editModal.loading}
    <div class="space-y-3" role="status" aria-label="Menyiapkan formulir kelas"><div class="h-11 animate-pulse rounded-lg bg-slate-100"></div><div class="h-11 animate-pulse rounded-lg bg-slate-100"></div></div>
  {:else}
