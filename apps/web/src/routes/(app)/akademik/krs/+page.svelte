...
<form method="GET" class="filter-panel my-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4" use:seamlessFilter>
  ...
  <details class="sm:col-span-2 lg:col-span-4"><summary class="cursor-pointer text-sm font-semibold text-slate-600">Pilihan filter lainnya</summary><div class="mt-4 grid gap-4 sm:grid-cols-2"><label class="text-sm font-semibold">Cari pilihan semester<input name="term_search" value={page.url.searchParams.get('term_search') ?? ''} class="control-base mt-1.5" /></label>
  <label class="text-sm font-semibold">Cari pilihan program<input name="program_search" value={page.url.searchParams.get('program_search') ?? ''} class="control-base mt-1.5" /></label></div></details>
...
{#if data.terms.meta.total > data.terms.meta.limit || data.programs.meta.total > data.programs.meta.limit}<details class="mb-4 text-sm text-slate-600"><summary class="cursor-pointer font-semibold">Jelajahi pilihan filter</summary><p class="mt-3">Semester</p><Pagination {...data.terms.meta} href={value => href('term_page', value)} /><p>Program studi</p><Pagination {...data.programs.meta} href={value => href('program_page', value)} /></details>{/if}
