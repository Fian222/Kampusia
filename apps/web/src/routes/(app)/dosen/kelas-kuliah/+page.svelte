...
<div class="mt-6 flex items-end justify-between gap-4"><div><p class="eyebrow">Penugasan Anda</p><h2 class="mt-1 text-lg font-semibold text-slate-900">{data.records.meta.total} kelas</h2></div></div>
<section class="table-shell relative mt-3 overflow-x-auto" aria-busy={listPending}><ListPending /><table class="min-w-[700px] w-full text-left text-sm transition-opacity" class:opacity-80={listPending}><thead><tr><th class="p-4">Mata kuliah</th><th class="p-4">Kelas</th><th class="p-4">Semester</th><th class="p-4">Status</th><th class="p-4">Tindakan</th></tr></thead>
  <tbody class="divide-y divide-slate-100">{#each data.records.data as row}<tr><td class="p-4"><span class="font-mono text-xs font-semibold text-slate-500">{row.mataKuliah.kode}</span><p class="mt-1 font-semibold text-slate-900">{row.mataKuliah.nama}</p><p class="mt-1 text-xs text-slate-500">{row.mataKuliah.sks} SKS · {row.programStudi.nama}</p></td><td class="p-4"><span class="inline-flex size-8 items-center justify-center rounded-lg bg-slate-100 font-bold">{row.namaKelas}</span></td><td class="p-4">{row.semester.nama}</td><td class="p-4"><Badge tone={row.status === 'DIBUKA' ? 'success' : row.status === 'DIBATALKAN' ? 'danger' : 'neutral'}>{row.status}</Badge></td><td class="p-4"><a class="action-primary" href={`/dosen/kelas-kuliah/${row.id}`}>Buka kelas <Icon name="arrow-right" size={15} /></a></td></tr>{:else}<tr><td colspan="5" class="p-8 text-center text-slate-500">Belum ada kelas yang ditugaskan.</td></tr>{/each}</tbody>
