  ...
  <aside class="border-b border-slate-200 bg-white lg:fixed lg:inset-y-0 lg:w-64 lg:border-r lg:border-b-0">
    ...
    <nav id="sidebar-nav" class="px-4 pb-6 lg:block" class:hidden={!menuOpen} aria-label="Navigasi utama">
      ...
      {#if data.user.role === 'ADMIN' || data.user.role === 'AKADEMIK'}
        {#each [{ path: '/akademik/fakultas', label: 'Fakultas' }, { path: '/akademik/program-studi', label: 'Program Studi' }, { path: '/akademik/mahasiswa', label: 'Mahasiswa' }, { path: '/akademik/dosen', label: 'Dosen' }, { path: '/akademik/mata-kuliah', label: 'Mata Kuliah' }, { path: '/akademik/kurikulum', label: 'Kurikulum' }] as item}
