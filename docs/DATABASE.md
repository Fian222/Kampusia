...
The grading tables and their application APIs, service policies, and frontend workflows are implemented. Normal grading is owned by assigned DOSEN, while ADMIN/AKADEMIK provide read-only oversight and controlled post-finalization correction. This document must be updated before any later design change and implemented schema must be checked against it. See [database package notes](../packages/db/README.md) for verification commands and the boundary between database constraints and service rules.
...
- ADMIN/AKADEMIK own meeting metadata: only these roles may create, edit, factually correct, or cancel a pertemuan. An assigned DOSEN may read meetings for classes they teach but cannot mutate the meeting number, date, time, material, class ownership, or lifecycle cancellation state.
...
- Creation by ADMIN/AKADEMIK produces TERJADWAL and is the operational act of opening the meeting for attendance. The persisted model has no separate draft/open state; UI wording such as “Buka Pertemuan” does not introduce another database lifecycle.
...
- While a meeting is TERJADWAL, recording and in-place correction require an active authorized ADMIN/AKADEMIK account or a DOSEN account linked to an active lecturer assigned to the class. After completion, normal DOSEN editing is read-only and only ADMIN/AKADEMIK may perform the controlled correction paths. Later account deactivation or lecturer reassignment does not alter the recorded actor references.
...
- Before any result for the class is finalized, an active DOSEN assigned through kelas_dosen may create components and edit nama, bobot, urutan, or is_active. ADMIN/AKADEMIK have read-only oversight of ordinary configuration. Changes after score entry are allowed only before finalization, must be explicit, and immediately change the nonofficial preview calculation; existing score values remain attached to the same stable component id.
...
- Before class finalization, only an active DOSEN assigned through kelas_dosen may enter and correct ordinary scores. ADMIN/AKADEMIK have read-only oversight during ordinary grading. A score update preserves dicatat_oleh and created_at while updating diubah_oleh and updated_at. Explicitly clearing a mistaken pre-final score sets nilai to null rather than deleting the row.
...
- Normal finalization belongs to an active assigned DOSEN. Where a coordinator exists, only that coordinator may finalize. With exactly one active assigned DOSEN and no coordinator, that lecturer may finalize. With multiple active assigned DOSEN and no coordinator, finalization is blocked until ADMIN/AKADEMIK explicitly designate one; the application never selects one implicitly. ADMIN and AKADEMIK cannot initiate normal finalization. The actor must have an active account at action time, and authorization is always enforced server-side.
...
ADMIN/AKADEMIK create and manage meeting metadata. Creating a TERJADWAL meeting is also the act of making it available to assigned lecturers; no distinct persisted open status exists. An assigned active DOSEN may view those meetings, record the four explicit attendance statuses, and complete attendance after every effective-roster student has a row, but cannot create, edit, or cancel meeting metadata. Once SELESAI, the DOSEN view is read-only. ADMIN/AKADEMIK retain the existing factual meeting correction and justified attendance correction/late-entry paths.
